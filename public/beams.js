/**
 * Beams — 动态光束背景
 * 从 React @react-three/fiber 组件转换为 vanilla Three.js
 * 使用 ShaderMaterial + 3D simplex noise 驱动顶点位移
 */

import * as THREE from 'three';

/* ============================================
   GLSL — Noise & Shader Headers
   ============================================ */
...
const glslVertexHelpers = /* glsl */ `
float getPos(vec3 pos) {
  vec3 noisePos = vec3(pos.x * 0., pos.y - uv.y, pos.z + uTime * uSpeed * 3.) * uScale;
...
export function initBeams(canvas, options = {}) {
  const {
    beamWidth = 2,
    beamHeight = 15,
    beamNumber = 12,
  ...
  // ---- Material — 基于 THREE.ShaderLib.physical 的自定义 ShaderMaterial ----
  const baseLib = THREE.ShaderLib.physical || THREE.ShaderLib.standard;
  ...
  uniforms.uScale = { value: beamScale };

  // 顶点着色器：注入 noise + 顶点位移 + 法线重算
  let vertexShader = `${glslHeader}\n${glslVertexHelpers}\n${baseLib.vertexShader}`;
  ...
  // 片元着色器：注入噪点颗粒
  let fragmentShader = `${glslHeader}\n${baseLib.fragmentShader}`;
  ...
  return { scene, renderer, material, cleanup };
}
